...
2. Pick a branch name: short and kebab-case, with the same prefix as `git branch --show-current` if it has one (e.g. `iain/`).
...
4. From the repo, run:
   ...
   ```bash
   ${CLAUDE_SKILL_DIR}/spinoff <branch> <<'BRIEF'
   <the brief>
   BRIEF
   ```

   It creates the worktree as a herdr workspace, starts Claude there on the brief (saved in the worktree's git dir, so it's never committed), and prints the worktree path.
