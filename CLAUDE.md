...
4. mise — installs mise itself from `https://mise.run` (never Homebrew: a brewed mise can't `self-update`), then `mise install` for the tools pinned in `config/mise/config.toml`, e.g. `hk` for git hooks (auto-detected when that config exists, skip with `--no-mise`). jdx's tools — `hk`, `pitchfork`, `aube`, `fnox` — belong here rather than in the `Brewfile`
...
- **`config/mise/config.toml`** — Global mise settings plus the pinned `[tools]` (languages and jdx's own CLIs)
