...
The top of `config/fish/config.fish` sets all four XDG base dirs explicitly (standard paths) and points tools that ignore XDG on macOS at them: Homebrew, npm, pnpm, cargo/rustup, mr-boxington (`mbx`), Go, pip, bundler, NuGet, Playwright, Maven. Split by kind — credentials and settings under `XDG_CONFIG_HOME`, installs under `XDG_DATA_HOME`, anything re-downloadable under `XDG_CACHE_HOME`. mise, uv, aube and podman already follow XDG on their own.
