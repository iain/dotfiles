...
**Keep bootstrap config out of `config/mise/config.toml`.** That file is symlinked to the global `~/.config/mise/config.toml`, and mise merges `[bootstrap]`/`[dotfiles]` across the config hierarchy like `[tools]` — anything bootstrap-related there would be applied by `mise bootstrap` in every project on the machine. The global file holds only `[settings]`, `[env]` (XDG and tool storage locations, see below) and `[tools]` (languages, jdx's CLIs: `hk`, `pitchfork`, `aube`, `fnox`, plus `kubectl`, `uv` and `yq`); it still merges into a bootstrap run from this repo.

**mise tools vs Homebrew packages.** A tool goes in mise `[tools]` when projects pin its version (`kubectl` must match each cluster) or mise itself depends on it (`uv`, for `pipx.uvx`). It stays a `brew:` package when something that doesn't go through mise runs it: vim launched from the Dock (`ripgrep`, `fd`, `fzf`, `jq`), git run by GUI apps (`git-delta`, `difftastic`), `config.fish` before `mise activate` (`starship`, `zoxide`, `direnv`), and `gh`, which mise's `credential_command` calls (as a mise tool its shim would re-enter mise). Libraries and services stay in Homebrew too. Tools a project pins in its own `mise.toml` are only active inside that project; when one is also wanted globally and only used from the terminal, give it a global mise default (`yq`) rather than a Homebrew copy, which the project pin then overrides. Homebrew packages a project installs itself (the monorepo's Brewfile: `pgvector`, `graphviz`, `text-embeddings-inference`) are deliberately not declared here, which means `mise bootstrap packages prune` run from this repo would remove them.
...
- **`claude/`** — Claude Code config symlinked into `~/.claude`: `settings.json` (permissions, status line, `SessionStart` hook, plus the enabled plugins and known marketplaces that drive the `setup-claude` task), `statusline.sh` (hostname-led status line), `hooks/machine-context.sh` (a `SessionStart` hook that injects the hostname into the model's context so it knows which machine it's on), `hooks/worktree-create.sh` and `hooks/worktree-remove.sh` (`WorktreeCreate`/`WorktreeRemove` hooks that put Claude's worktrees in herdr's `~/Code/worktrees/<repo>/<branch-slug>` layout, through herdr when inside it and plain git otherwise; the creation logic lives in `bin/worktree-create`, shared with the skill below), `skills/spinoff/` (the user-only `/spinoff` skill: writes a brief of a bug or task from the current session and hands it to a new Claude session in a fresh herdr worktree workspace), and `bin/rubocop-mcp` (a per-repo-adaptive rubocop MCP launcher, registered user-scope by the `setup-claude` task). Note: `settings.json` is a tracked file but Claude also writes to `~/.claude/settings.json` at runtime (theme, plugin state), so machine-local tooling that rewrites it — e.g. peon-ping — is intentionally **not** tracked here; its hooks land in `~/.claude/settings.json.backup` after a bootstrap run
...
**Nothing user-writable runs as root.** The supervisor runs as the user from the `dev.mise.pitchfork` LaunchAgent, via `bin/pitchfork-supervisor`, which sets a usable `PATH` and runs `mise x -- pitchfork supervisor run --boot` — so it always uses the newest installed pitchfork and gets mise's `[env]`. Unprivileged processes can't bind 443 on macOS, so the proxy listens on 8443 and a root-owned pf rule forwards loopback 443 to it: `system/etc/pf.anchors/dotfiles-pitchfork`, loaded into the `com.apple/250.dotfiles-pitchfork` anchor by the `dev.dotfiles.pitchfork-pf` LaunchDaemon, which runs only `/bin/sh` and `/sbin/pfctl`.

- **The proxy port is set in the wrapper only** (`PITCHFORK_PROXY_PORT=8443`, plus `PITCHFORK_PROXY_SYNC_HOSTS=false` since `/etc/hosts` needs root and macOS resolves `*.localhost` itself). Pitchfork builds the URLs it prints from that port, and **8443 isn't reachable directly while the forward is active** — only the forwarded 443 is — so shells must keep seeing the default 443. For the same reason mise's `[env]` sets `PITCHFORK_SUPERVISOR_AUTO_START=false`: a client command must never start its own supervisor on the wrong port.
...
- **Restart with `pfr`** (`launchctl kickstart -k gui/$UID/dev.mise.pitchfork`), e.g. after upgrading pitchfork. It's both a fish abbreviation, usable anywhere, and the `restart-pitchfork` task (alias `pfr`) in this repo. `pitchfork supervisor start --force` replaces it with a copy launchd doesn't manage.
- **If `https://*.localhost` stops answering**, first check the supervisor is up (`launchctl print gui/$UID/dev.mise.pitchfork`, `lsof -a -nP -iTCP:8443 -sTCP:LISTEN`), then the forward: `sudo pfctl -s info` and `sudo pfctl -a com.apple/250.dotfiles-pitchfork -s nat`; re-run it with `sudo launchctl kickstart system/dev.dotfiles.pitchfork-pf`. Other software that disables pf would do this. Don't test via `:8443` — it's unreachable by design while the forward works.
