...
# NOTE: Documentation mentions options that donâ€™t exist anymore, for example,
...
# TODO: Shouldnâ€™t we add a __git_branches type completion?
...
# is, %(refname), %(objecttype), %(objectsize), %(objectname) with optional â€˜*â€™
# in front.
...
_git-branch () {
  ...
  # TODO: Iâ€™m not happy with having to force a pattern with -A.
  _arguments -S -A "-*" \
    - list \
    ...
    - modify \
      ...
      '--set-upstream[set up configuration so pull merges from the start point for existing branches]' \
      $dependent_modification_args \
    - delete \
...
__git_config_gettable_name () {
  ...
  # as well.  However, thereâ€™s no good way to tell from the output of
  # git-config, so thisâ€™ll have to do until we write our own .git/config
  # parser (which will never happen because itâ€™s not worth the trouble).
...
# TODO: Itâ€™d be really cool if both the default and the current value could be
...
__git_config_values () {
  ...
  case $1 in
    ...
    ((#i)gitcvs.(*.|)allbinary)
      ...
      ;;
    ((#i)gitcvs.dbname)
      # TODO: In the future, when computers are self-aware and this wonâ€™t
    ...
    ((#i)gitcvs.dbdriver)
      ...
      # couldnâ€™t figure out a good way of doing that when I wrote this code.
      drivers=(
